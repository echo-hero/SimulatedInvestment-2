...
//    FundData firstByFundcodeAndFunddate(String fundCode, String fundDate);
    ...
    @Select(" select * from fund_data where fundcode = #{fundCode} order by funddate desc limit 0,1 ")
    FundData firstNewByFundCode(String fundCode);
    ...
    @Select(" select * from fund_data where fundcode = #{fundCode} order by funddate desc ")
